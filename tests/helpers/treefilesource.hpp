...
class TreeFileSource : public rex::FileSource<Tree>
{
    public:
        ...
        Tree loadFromFile(const rex::Path& path) const override
        {
            std::ifstream treeFile(path);

            if(!treeFile)
                throw rex::InvalidResourceException("cannot open tree file '" + path.str() + "'");
            ...
            size_t numberStartAt = path.str().find_first_of("1234567890");
            std::string startsWithNumber = path.str().substr(numberStartAt);
